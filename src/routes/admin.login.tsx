...
function AdminLogin() {
  ...
  useEffect(() => {
    let cancelled = false;
    queryClient.fetchQuery(adminMeQueryOptions()).then((profile) => {
      if (!cancelled && profile) navigate({ to: "/admin" });
    });
    return () => {
      cancelled = true;
    };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Re-check admin status server-side (a valid Supabase user isn't necessarily an admin).
      const profile = await queryClient.fetchQuery(adminMeQueryOptions());
      if (!profile) {
